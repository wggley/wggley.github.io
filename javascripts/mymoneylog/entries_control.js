...
mlog.entriesControl = function() {
  ...
  _dtStart.setDate(_dtStart.getDate() - 2);
  ...
  return {
    ...
    init: function() {
      if (!_htmlTemplate) {
        ...
        $('#input_value').calculator({
          layout: ['MC_7_8_9_/' + $.calculator.CLOSE, 'MR_4_5_6_*' + $.calculator.USE,
        ...
        $('#input_description').autocomplete(mlog.entries.getDescriptions(), {
          selectFirst: false
        });
        ...
        $('#input_account').autocomplete(mlog.accounts.getNames(), _acOptions).result(function() {
          /* on accept jump to: */
        ...
        $('#input_account_to').autocomplete(mlog.accounts.getNames(), _acOptions).result(function() {
          /* on accept jump to: */
          if (!$.browser.opera)
            $('#form_entry button.msg')[0].focus();
        });
        ...
        resetFilterOptions();
        mlog.entriesControl.clearEntry();
      }
    },
    ...
    prepareRowEdit: function(elem, lineId) {
      if (lineId == undefined) {
        lineId = elem.parentNode.parentNode.getAttribute('id') || '';
      }
      if (lineId == '') {
      ...
      $('#input_description_row').autocomplete(mlog.entries.getDescriptions(), {
        selectFirst: false
      }).val(_description);
    ...
    show: function(page) {
      ...
      $('#entries_table .ac_input').each(function() {
        $(this).unautocomplete();
      });
    ...
    removeAll: function(recursive) {
      var callBack = function() {
        var id, el, iterationObject = $('#entries_table tr .opt_delete').first();
        var el = iterationObject.parent().parent();
        el.addClass('bg_delete');
        var lineId = el.attr('id').substring(2);
        mlog.entries.remove(lineId);
        mlog.entriesControl.show();
        mlog.entriesControl.updateTagCloud();
        //el.fadeOut("normal", function() {} );
        if ($('#entries_table tr .opt_delete').length > 0) {
          callBack();
        }
      };
      mlog.entriesControl.showConfirmation(
        mlog.translator.msg('confirmation').toUpperCase(),
        mlog.translator.msg('delete_all').toUpperCase() + ' <br/> ' + mlog.translator.msg('are you sure?'),
        callBack
      );
    },
    reconcileAll: function() {
      var callBack = function() {
        var iterationObject = $('.row_reconcilable .opt_ok');
        iterationObject.each(
          function(index, element) {
            mlog.entriesControl.reconcileEntry(element, true);
          }
        );
		iterationObject.each(
		  function(index, element) {
			id = element.parentNode.parentNode.getAttribute('id').substring(2);
			$('#n_' + id).addClass('new_entry');
          }
        );
      };
      mlog.entriesControl.showConfirmation(mlog.translator.msg('confirmation').toUpperCase(), mlog.translator.msg('conciliate_all').toUpperCase() + ' <br/> ' + mlog.translator.msg('are you sure?'), callBack);

    },
    /* show pop-up to edit all visible columns*/
    showConfirmation: function(title, message, okCallback) {
      $.modal.close();
      $('.confirmation_modal:hidden .entries_text').html(title);
      $('.confirmation_modal:hidden #text').html(message);
      $('.confirmation_modal:hidden .button').unbind('click').click(
        function() {
          $.modal.close();
        }
      );

      $('.confirmation_modal:hidden .button.ok').click(okCallback);

      $('.confirmation_modal:hidden').modal({
        containerId: 'confirmationContainerModalId',
    ...
    showEditAll: function() {
      $('.vmenu:hidden.edit_all_modal .button').unbind('click').click(
        function() {
          $.modal.close();
        }
      );

      $('.vmenu:hidden.edit_all_modal').modal({
        containerId: 'editAllContainerModalId',
        onShow: function(dialog) {
          $('#input_date_all').jscalendar();
          $('#input_value_all').calculator({
            layout: ['MC_7_8_9_/' + $.calculator.CLOSE, 'MR_4_5_6_*' + $.calculator.USE, 'MS_1_2_3_-BS', 'M+_0_._=_+' + $.calculator.CLEAR],
            showOn: 'button',
            buttonImageOnly: true,
            buttonImage: 'images/transparent.png',
            constrainInput: false,
          ...
          $('#input_description_all').autocomplete(mlog.entries.getDescriptions(), {
            selectFirst: false
          }).val();
          $('#input_category_all').autocomplete(mlog.categories.getNames(), _acOptionsMulti).val();
          $('#input_account_all').autocomplete(mlog.accounts.getNames(), _acOptions).val();
        },
        onOpen: function(dialog) {
          dialog.overlay.fadeIn(
            100,
    ...
    editAll: function() {
      var id, entry = [],
        entriesToAdd = [];
      entry[0] = $('#input_date_all').val();
      var isPending = $('#input_pending_all').prop('checked') || entry[0].indexOf('?') > -1;
      entry[1] = $('#input_value_all').val();
      entry[2] = $('#input_description_all').val();
      entry[3] = $('#input_category_all').val();
      entry[4] = $('#input_account_all').val();

      var callBack = function() {
        var element = $('#entries_table .highlt').first();
        id = element.attr('id').substring(2);
        var newEntry = [],
          currentEntry = mlog.entries.getById(id);

        newEntry[0] = entry[0] == "" ? currentEntry[0] : entry[0];
        newEntry[1] = "" + (entry[1] == "" ? currentEntry[1] : entry[1]);
        newEntry[2] = entry[2] == "" ? currentEntry[2] : entry[2];
        newEntry[3] = entry[3] == "" ? currentEntry[3] : entry[3];
        newEntry[4] = entry[4] == "" ? currentEntry[4] : entry[4];
        newEntry[5] = '';
        newEntry[0] = newEntry[0] + (isPending || currentEntry[6] ? '?' : '');
        mlog.entries.remove(id);
        entriesToAdd.push(newEntry);
        mlog.entriesControl.show();
        mlog.entriesControl.updateTagCloud();
        if ($('#entries_table .highlt').length > 0) {
          return callBack();
        }
        for (i = 0; i < entriesToAdd.length; i++) {
          mlog.entries.add(entriesToAdd[i]);
        }
        /* refresh entries */
        mlog.entriesControl.show();
        mlog.entriesControl.updateTagCloud();
        $('#entries_table .highlt').addClass('new_entry');
        /* initial state and update autocompleters */
        $('#transfer').hide();
      }
      callBack();
    },
    reconcileEntry: function(elem, all) {
      if (all == undefined) {
        all = false;
      }
      var id = elem.parentNode.parentNode.getAttribute('id').substring(2);
      if (!all) {
        confirm(mlog.translator.msg('conciliate').toUpperCase() + ': ' + mlog.translator.msg('are you sure?'));
      }
